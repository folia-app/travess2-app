...
const isNotMinted = computed(() =>
  !store.state.nfts
    ? undefined
    : store.state.nfts.find((token) => token.tokenId === tokenId) === undefined
)

// const iframePath = import.meta.env.VITE_DEV_IFRAME_PATH ?? ipfsIframe
const iframePath = '/iframe.html?ipfspath=' + encodeURIComponent(import.meta.env.VITE_IPFS_PATH)
...
<template>
  <article
    class="fixed z-30 overlay bg-[rgba(0,0,0,0.85)] overflow-hidden p-9 md:p-12 flex cursor-pointer"
  >
    <button
      ref="closeOverlayButton"
      class="absolute overlay outline-none"
      @click="$router.go(-1)"
    ...
    <div class="relative w-full z-10" @click.stop>
      <!-- (loading) -->
      <div
        class="absolute overlay flex items-center justify-center bg-black"
      ...
      <template v-if="isNotMinted === false">
        <iframe
          ...
          :src="`${iframePath}#${tokenId}-${VITE_IPFS_HASH_IMAGES}`"
          @load="onIframeLoad"
        />
      </template>
    </div>
