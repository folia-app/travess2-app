...
import { readProvider, lookupAddresses } from './rpc'
...
const store = createStore({
  ...
  actions: {
    /**
     * One Addr component asks for one name, but a page mounts twenty-five of
     * them in the same tick. Rather than change every call site, requests made
     * close together are collected and answered by one batched reverse lookup:
     * two eth_calls for the whole page instead of roughly four per address.
     */
    async ensName({ state, commit }, addr) {
      ...
      try {
        ...
        // Commit every name the batch resolved, not only the one asked for, so
        // the sibling components do not each repeat the round trip.
